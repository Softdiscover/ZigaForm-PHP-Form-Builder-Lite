...
class elFinderVolumeDropbox2 extends elFinderVolumeDriver {
	...
	public static function getTokenFromOauth1( $app_key, $app_secret, $oauth1_token, $oauth1_secret ) {
		...
		$res = $result ? json_decode( $result, true ) : array();
	...
	public function netmountPrepare( $options ) {
		...
		try {
			...
			if ( $options['pass'] === 'reauth' ) {
				$options['pass'] = '';
				$this->session->set( 'Dropbox2AuthParams', array() )->set( 'Dropbox2Tokens', array() );
			} elseif ( $options['pass'] === 'dropbox2' ) {
				$options['pass'] = '';
			}

			$options = array_merge( $this->session->get( 'Dropbox2AuthParams', array() ), $options );

			if ( ! isset( $options['tokens'] ) ) {
				$options['tokens'] = $this->session->get( 'Dropbox2Tokens', array() );
	...
	protected function _save( $fp, $path, $name, $stat ) {
		try {
			...
			return $this->service->upload( $dropboxFile, $fullpath, array( 'mode' => 'overwrite' ) )->getPathLower();
		} catch ( DropboxClientException $e ) {
			return $this->setError( 'Dropbox error: ' . $e->getMessage() );
		}
	}
	...
	protected function _filePutContents( $path, $content ) {
		...
		if ( $local = $this->getTempFile( $path ) ) {
			if ( file_put_contents( $local, $content, LOCK_EX ) !== false
				...
				$res = $this->_save( $fp, $path, $name, array() );
				fclose( $fp );
			}

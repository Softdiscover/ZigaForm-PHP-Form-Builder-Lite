...
class elFinder {
	...
	public function __construct( $opts ) {
		...
		// set archivers
		self::$archivers = isset( $opts['archivers'] ) && is_array( $opts['archivers'] ) ? $opts['archivers'] : array();
	...
	protected function abort( $args = array() ) {
		if ( ! self::$connectionFlagsPath || $_SERVER['REQUEST_METHOD'] === 'HEAD' ) {
			return;
		}
	...
	public static function expandMemoryForGD( $imgInfos ) {
		if ( self::$memoryLimitGD != 0 && $imgInfos && is_array( $imgInfos ) ) {

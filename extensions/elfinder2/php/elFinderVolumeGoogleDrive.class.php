...
class elFinderVolumeGoogleDrive extends elFinderVolumeDriver {
	...
	private function _gd_getFile( $path, $fields = '' ) {
		...
		try {
			$file = $this->service->files->get( $itemId, array( 'fields' => $fields ) );
	...
	protected function _gd_getDirectoryData( $usecache = true ) {
		if ( $usecache ) {
			$cache = $this->session->get( $this->id . $this->netMountKey, array() );
			if ( $cache ) {
	...
	protected function _gd_getMountPaths( $id ) {
		...
		if ( $root ) {
			if ( $this->root === '/' || strpos( '/' . $path, $this->root ) === 0 ) {
				$res = array( ( strpos( $path, '/' ) === false ) ? '/' : ( '/' . $path ) );
			}
		} else {
	...
	protected function _gd_getThumbnail( $path ) {
		...
		try {
			$contents = $this->service->files->get(
				$itemId,
				array(
					'alt' => 'media',
				)
	...
	public function netmountPrepare( $options ) {
		...
		try {
			...
			if ( $options['pass'] === 'reauth' ) {
				$options['pass'] = '';
				$this->session->set( 'GoogleDriveAuthParams', array() )->set( 'GoogleDriveTokens', array() );
			} elseif ( $options['pass'] === 'googledrive' ) {
				$options['pass'] = '';
			}

			$options = array_merge( $this->session->get( 'GoogleDriveAuthParams', array() ), $options );

			if ( ! isset( $options['access_token'] ) ) {
				$options['access_token'] = $this->session->get( 'GoogleDriveTokens', array() );
			...
			if ( isset( $options['user'] ) && $options['user'] === 'init' ) {
				...
				if ( ! $aToken && empty( $_GET['code'] ) ) {
					...
					$folders         = array( 'root' => $rootObj->getName() ) + $folders;
			...
			if ( empty( $options['pass'] ) ) {
				...
				return array(
					'exit'  => true,
					'error' => array( elFinder::ERROR_ACCESS_DENIED, $e->getMessage() ),
				);
			}
		}
		...
		try {
			...
			if ( isset( $err['error'] ) && $err['error']['code'] == 404 ) {
				return array(
					'exit'  => true,
					'error' => array( elFinder::ERROR_TRGDIR_NOT_FOUND, $options['path'] ),
				);
			} else {
				return array(
					'exit'  => true,
		...
		foreach ( array( 'host', 'user', 'pass', 'id', 'offline' ) as $key ) {
			unset( $options[ $key ] );
		}
	...
	protected function doSearch( $path, $q, $mimes ) {
		...
		if ( $itemId !== 'root' ) {
			$dirs  = array_merge( array( $itemId ), $this->_gd_getDirectories( $itemId ) );
	...
	protected function _copy( $source, $targetDir, $name ) {
		...
		try {
			...
			$file          = $this->service->files->copy( $srcId, $file, array( 'fields' => self::FETCHFIELDS_GET ) );
			$itemId        = $file->id;
	...
	protected function _save( $fp, $path, $name, $stat ) {
		...
		try {
			...
			if ( $size > 0 ) {
				...
				if ( $memory > 0 ) {
					$chunkSizeBytes = max( 262144, min( array( $chunkSizeBytes, ( intval( $memory / 4 / 256 ) * 256 ) ) ) );
				}
			}
